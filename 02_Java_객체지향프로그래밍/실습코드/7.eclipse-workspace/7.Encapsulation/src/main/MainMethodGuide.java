...
public class MainMethodGuide {
	...
	public static void main(String[] args) {
		staticReason();
		commandLineArguements(args);
		
	}

	private static void commandLineArguements(String[] args) {
		//명령행 인수(args) 활용
		...
		// 명령행 인수 처리 (패턴화)
		String mode = args.length > 0 ? args[0] : "default";	//첫번째 인수
	...
	private static void staticReason() {
		...
		System.out.println(" - JVM이 클래스 로딩시 객체 생성 없이 바로 호출 해야 함.");
		System.out.println(" - 프로그램 시작점이므로 인스턴스화 과정이 불필요");
		
		// static 메서드에서는 static 변수만 직접 접근 가능
