...
SELECT * FROM EMP WHERE SAL <> 3000;	-- <> 연산자 (표준sql)
SELECT * FROM EMP WHERE SAL ^= 3000;    -- ^= 연산자 (Oracle 전용)
SELECT * FROM EMP WHERE NOT SAL = 3000;	-- Not 연산자 사용
	
-- IN 연산자
SELECT * FROM EMP
WHERE JOB IN ('MANAGER', 'SALESMAN', 'CLERK'); -- 지정된 값 중 하나와 일치 
...
-- 10번과 30번 부서 직원만 조회하시오. (부서번호, 이름순으로 정렬하시오)
SELECT ENAME,
		JOB,
		DEPTNO,
		SAL
...
-- 급여가 정확히 1250, 1500, 3000인 직원을 조회하시오.(정렬은 급여순으로하시오)
SELECT ENAME,
		JOB,
		DEPTNO,
		SAL
...
-- 상사가 7698, 7839, 7902인 직원을 조회하시오.(정렬은 상사, 이름순으로 하시오)
SELECT ENAME,
		JOB,
		DEPTNO,
		SAL,
...
-- 20번, 30번 부서의 MANAGER와 ANALYST만 직원을 조회하시오.
-- 정렬은 부서번호, 직책으로 하시오.
SELECT ENAME,
       JOB,
       DEPTNO,
