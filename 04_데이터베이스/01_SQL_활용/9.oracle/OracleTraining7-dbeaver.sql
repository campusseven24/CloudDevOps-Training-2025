...
SELECT sysdate,		-- 현재 시스템 날짜/시간
		...
		ROUND(SYSDATE, 'Q') AS FORMAT_Q, -- 분기 단위 반올림, 각분기 중간지점 기준
...
-- CC: 세기, YYYY: 년, Q: 분기, DDD: 일, HH: 시간  
SELECT sysdate,		-- 현재 시스템 날짜/시간
		TRUNC(sysdate, 'CC') AS FORMAT_CC, -- 세기단위 버림,50년 기준으로 반올림
		TRUNC(SYSDATE, 'YYYY') AS FORMAT_YYYY, -- 년단위 버림, 7월 1일 기준
		TRUNC(SYSDATE, 'Q') AS FORMAT_Q, -- 분기 단위 버림, 각분기 중간지점 기준
		TRUNC(SYSDATE, 'DDD') AS FORMAT_DDD, --- 일 단위 버림, 정오(12:00) 기준
		TRUNC(SYSDATE, 'HH') AS FORMAT_HH -- 시간 단위 버림, 30분 기준 
FROM dual;
...
SELECT 'ABCD' + EMPNO, EMPNO   -- 실행시 오류 발생!
FROM EMP 
WHERE ename = 'SCOTT';
...
SELECT sysdate,			-- 현재 시스템 날짜/시간
	...
	TO_CHAR(sysdate, 'MON') AS MON, -- 월(약어)을 3자 약어로 표시 (한글: '8월')
	...
	TO_CHAR(SYSDATE, 'DAY') AS DAY -- 요일을 전체 이름으로 표시 (공백으로 9자리 채움) (한글: '수요일')
FROM dual;
...
SELECT TO_CHAR(TO_DATE('49/12/10', 'YY/MM/DD'), 'YYYY-MM-DD') AS YY_YEAR_49,
		...
		-- RR 형식: 50 -> 50~99 범위이므로 1900년대 => 1950년 (차이점)
FROM DUAL;
...
SELECT 
	...
	NEXT_DAY(TO_DATE(TO_CHAR(SYSDATE, 'YYYY') || 
						TO_CHAR(HIREDATE, '-MM-DD')),'월요일')
	AS "올해_입사기념일_다음_월요일"
FROM EMP;
